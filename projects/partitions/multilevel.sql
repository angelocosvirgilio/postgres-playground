...
CREATE TABLE partitions.customers (id INTEGER, status TEXT, arr NUMERIC) PARTITION BY LIST(status);
    ...
    CREATE TABLE partitions.cust_active PARTITION OF partitions.customers FOR VALUES IN ('ACTIVE','RECURRING','REACTIVATED') PARTITION BY RANGE(arr);
        ...
        CREATE TABLE partitions.cust_active_def PARTITION OF partitions.cust_active DEFAULT;
            
    CREATE TABLE partitions.cust_other PARTITION OF partitions.customers DEFAULT PARTITION BY RANGE(arr);
        CREATE TABLE partitions.cust_arr_large PARTITION OF partitions.cust_other FOR VALUES FROM (101) TO (MAXVALUE) PARTITION BY HASH(id);
        ...
        CREATE TABLE partitions.cust_other_def PARTITION OF partitions.cust_other DEFAULT;


INSERT INTO partitions.customers VALUES (1,'ACTIVE',100), (2,'RECURRING',20), (3,'REACTIVATED',38),(4,'EXPIRED',120), (5,'ACTIVE',200), (6,'EXPIRED',80);
